...
class Player extends Actor {
  ...
  set isMoving (moving: boolean) {
    ...
    if (this._isMoving) {
      ...
      this.walkAnimation[this.direction].goToFrame((this.step === Step.First) ? 0 : 4)
  ...
  constructor (config?: ActorArgs) {
    ...
    super({ ...config, anchor: vec(0, 0), z: 2 })
  ...
  update (game: Engine, delta: number) {
    ...
    if (this.isMoving) {
      ...
      // check move command
      if (game.input.keyboard.isHeld(Input.Keys.Up)) {
        this.moveTarget = this.pos.add(vec(0, -48))
        ...
        this.moveTarget = this.pos.add(vec(-64, 0))
  ...
  setSheet () {
    ...
    // this.walkAnimation["test"] = Animation.fromSpriteSheet(playerWalkSheet[this.showingCharacterIndex],[0, 1, 2, 3, 2, 1,2, 3,2 , 0], 33,AnimationStrategy.Loop)
  }
}
